...
int main()
{
	...
	// Esto es solo porque la imagen de prueba no es multiplo de 32 en ambas dimensiones
	// Originalmente teniamos tamGrid (width/DIM_BLOQUE, height/DIM_BLOQUE); 
	// ********************************************************************
	//int gridx = width/DIM_BLOQUE + (width % DIM_BLOQUE == 0 ? 0 : 1);
	//int gridy = height/DIM_BLOQUE + (height % DIM_BLOQUE == 0 ? 0 : 1);
	//dim3 tamGrid (gridx, gridy); //Grid dimensión
	// ********************************************************************
	dim3 tamGrid (width/DIM_BLOQUE,height/DIM_BLOQUE);
