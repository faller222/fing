...
public class Reclamos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String proveedor = prov.getNickname();

            IReclamo IR = new IReclamo();
            IR.seleccionarProveedor(proveedor);
            List L = IR.listarReclamos();
            request.setAttribute("Lista", L);

            request.getRequestDispatcher("/WEB-INF/pages/verReclamos.jsp").forward(request, response);
            return;
        } catch (Exception e) {
